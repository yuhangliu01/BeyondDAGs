...
def main(args):
    ...
    clip_model = clip.load('RN101', jit=False, device=torch.device("cpu"))[0].cuda()
    clip_model.eval()
    ###################################################################################################################
    ...
    for train_step in range(1, len(train_dataiter) + 1):
        ...
        classname_list.append(batch["classname"])
    save_dir = os.path.join(cfg.OUTPUT_DIR, cfg.DATASET.NAME)
    os.makedirs(save_dir, exist_ok=True)
    save_filename = f"{args.split}" + "RN101"
    np.savez(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", type=str, default="/home/yuhang/CoOp-main/DATA", help="path to dataset")
    parser.add_argument("--output-dir", type=str, default="/home/yuhang/CoOp-main/DATA/clip_feat/", help="output directory")
    parser.add_argument("--config-file", type=str, default="/home/yuhang/CoOp-main/configs/trainers/CoOp/rn101.yaml", help="path to config file")
    parser.add_argument(
        "--dataset-config-file",
        type=str,
        default="/home/yuhang/CoOp-main/configs/datasets/imagenet.yaml",
        help="path to config file for dataset setup",
    )
    parser.add_argument("--num-shot", type=int, default=16, help="number of shots")
    parser.add_argument("--split", type=str,default="val", choices=["train", "val", "test"], help="which split")
