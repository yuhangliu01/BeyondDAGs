...
import torch
import sys
sys.path.append(os.path.abspath("/home/yuhang/CoOp-main"))
...
parser.add_argument("--dataset", type=str, default="ImageNet", help="path to dataset")
...
parser.add_argument("--feature_dir", type=str, default="/home/yuhang/CoOp-main/DATA/clip_feat", help="feature dir path")
...
imagenetv2_test_file = np.load(os.path.join("/home/yuhang/CoOp-main/DATA/clip_feat/ImageNetV2", "test.npz"))
imagenetv2_test_feature, imagenetv2_test_label = imagenetv2_test_file["feature_list"], imagenetv2_test_file["label_list"]
...
imagenetA_test_file = np.load(os.path.join("/home/yuhang/CoOp-main/DATA/clip_feat/ImageNetA", "test.npz"))
imagenetA_test_feature, imagenetA_test_label = imagenetA_test_file["feature_list"], imagenetA_test_file["label_list"]
...
imagenetR_test_file = np.load(os.path.join("/home/yuhang/CoOp-main/DATA/clip_feat/ImageNetR", "test.npz"))
imagenetR_test_feature, imagenetR_test_label = imagenetR_test_file["feature_list"], imagenetR_test_file["label_list"]

imagenetS_test_file = np.load(os.path.join("/home/yuhang/CoOp-main/DATA/clip_feat/ImageNetSketch", "test.npz"))
imagenetS_test_feature, imagenetS_test_label = imagenetS_test_file["feature_list"], imagenetS_test_file["label_list"]


os.makedirs("report", exist_ok=True)

val_shot_list = {1: 1}
clip_model = clip.load('ViT-B/16', jit=False, device=torch.device("cpu"))[0].cuda()


clip_weights = []

texts = [imagenet_templates.format(c.replace('_', ' ')) for c in imagenet_classes] #[t.format(classname) for t in imagenet_templates]
...
for num_shot in [1]:
    ...
    for seed in range(1, args.num_run + 1):
        ...
        # Disentangled matrix 
        # class_index = np.concatenate((fewshot_train_label,fewshot_val_label), axis=0)
        # text_list = []
        # train_vali_text = []
        ...
        # token = clip.tokenize(text_list).cuda()
        
        # print(token[0])
        # print(token.size())
        # text_features = np.empty((200,512))
        # for i in range(token.size(0)):
        #     text_feature =  clip_model.encode_text(token[i:i+1,:])
        #     text_features = np.append(text_features, text_feature.cpu().detach().numpy(), axis=0)
        # text_features_all = text_features[200:,:]
        pca = PCA(n_components=500) #500/160 1,2,4 -shot,4 500/200 4,8,16-shot
        

        ica = FastICA(n_components=160, random_state=0) # 200/200, 35.25 250/250 34.92  150/150 34.47 200/150 34.63 300/200 35.34 

        #default,300/250  35.03 300/200  35.03 400/200  35.35 500/200   500/150, 500/160 35.35

        #train_vali = np.concatenate((fewshot_train_feature,fewshot_val_feature, test_feature), axis=0)
        train_vali = np.concatenate((fewshot_train_feature,fewshot_val_feature), axis=0)
        
        train_vali_pca = pca.fit_transform(train_vali)
        feat_train_vali_ica =  ica.fit_transform(train_vali_pca)
        #mixing = ica.mixing_
        fewshot_train_feature_ica = feat_train_vali_ica[:np.size(fewshot_train_feature,0),:]
        fewshot_val_feature_ica = feat_train_vali_ica[np.size(fewshot_train_feature,0):np.size(fewshot_train_feature,0)+np.size(fewshot_val_feature,0),:]
        
        test_feature_pca = pca.transform(test_feature)
        test_feature_ica =  ica.transform(test_feature_pca)

        imagenetv2_test_feature_pca = pca.transform(imagenetv2_test_feature)
        imagenetv2_test_feature_ica =  ica.transform(imagenetv2_test_feature_pca)

        imagenetA_test_feature_pca = pca.transform(imagenetA_test_feature)
        imagenetA_test_feature_ica =  ica.transform(imagenetA_test_feature_pca)

        imagenetR_test_feature_pca = pca.transform(imagenetR_test_feature)
        imagenetR_test_feature_ica =  ica.transform(imagenetR_test_feature_pca)

        imagenetS_test_feature_pca = pca.transform(imagenetS_test_feature)
        imagenetS_test_feature_ica =  ica.transform(imagenetS_test_feature_pca)
        ...
        for c_weight in search_list:
            ...
            clf = LogisticRegression(solver="lbfgs", max_iter=1000, penalty="l2", C=c_weight).fit(fewshot_train_feature_ica, fewshot_train_label)
            pred = clf.predict(fewshot_val_feature_ica)
            acc_val = sum(pred == fewshot_val_label) / len(fewshot_val_label)
            acc_list.append(acc_val)

            #imagenetv2_pred = clf.predict(imagenetv2_test_feature)
            #imagenetv2_test_acc = 100 * sum(imagenetv2_pred == imagenetv2_test_label) / len(imagenetv2_test_label)
        ...
        def binary_search(c_left, c_right, seed, step, test_acc_step_list):
            clf_left = LogisticRegression(solver="lbfgs", max_iter=1000, penalty="l2", C=c_left).fit(fewshot_train_feature_ica, fewshot_train_label)
            pred_left = clf_left.predict(fewshot_val_feature_ica)
            acc_left = sum(pred_left == fewshot_val_label) / len(fewshot_val_label)
            print("Val accuracy (Left): {:.2f}".format(100 * acc_left), flush=True)

            clf_right = LogisticRegression(solver="lbfgs", max_iter=1000, penalty="l2", C=c_right).fit(fewshot_train_feature_ica, fewshot_train_label)
            pred_right = clf_right.predict(fewshot_val_feature_ica)
            ...
            pred = clf_final.predict(test_feature_ica)
            test_acc = 100 * sum(pred == test_label) / len(pred)
            print("Test Accuracy via ICA: {:.2f}".format(test_acc), flush=True)
            test_acc_step_list[seed - 1, step] = test_acc

            # Test on imagenetv2
            imagenetv2_pred = clf_final.predict(imagenetv2_test_feature_ica)
            imagenetv2_test_acc = 100 * sum(imagenetv2_pred == imagenetv2_test_label) / len(imagenetv2_test_label)
            print("Imagenetv2 Test Accuracy via ICA: {:.2f}".format(imagenetv2_test_acc), flush=True)

            # Test on imagenetA
            imagenetA_pred_prob = clf_final.predict_proba(imagenetA_test_feature_ica)[:,imagenet_a_mask]
            imagenetA_pred = imagenetA_pred_prob.argmax(1)
            imagenetA_test_acc = 100 * sum(imagenetA_pred == imagenetA_test_label) / len(imagenetA_test_label)
            print("ImagenetA Test Accuracy: {:.2f}".format(imagenetA_test_acc), flush=True)

            # Test on imagenetR
            imagenetR_pred_prob = clf_final.predict_proba(imagenetR_test_feature_ica)[:,imagenet_r_mask]
            imagenetR_pred = imagenetR_pred_prob.argmax(1)
            imagenetR_test_acc = 100 * sum(imagenetR_pred == imagenetR_test_label) / len(imagenetR_test_label)
            print("ImagenetR Test Accuracy: {:.2f}".format(imagenetR_test_acc), flush=True)

            # Test on imagenetS
            imagenetS_pred = clf_final.predict(imagenetS_test_feature_ica)
            imagenetS_test_acc = 100 * sum(imagenetS_pred == imagenetS_test_label) / len(imagenetS_test_label)
            print("ImagenetS Test Accuracy: {:.2f}".format(imagenetS_test_acc), flush=True)


            saveline = "{}, seed {}, {} shot, weight {}, test_acc {:.2f}\n".format(dataset, seed, num_shot, c_final, test_acc)
            with open(
                "{}_s{}r{}_details.txt".format(args.feature_dir, args.num_step, args.num_run),
                "a+",
            ) as writer:
    ...
    with open(
        "{}_s{}r{}.txt".format(args.feature_dir, args.num_step, args.num_run),
        "a+",
    ) as writer:
